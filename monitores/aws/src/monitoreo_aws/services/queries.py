...
DETAIL_QUERIES = {
    ...
    "mensajeria_errores_por_hora": """fields @timestamp, Httpcode, OperationInvokerName | filter Httpcode != 200 and OperationInvokerName in ['sendMessage','sendEMAIL','sendSMS','enviootp','EnviarSolicitud','ActualizarSolicitud'] | stats count(*) as count by bin(1h) as hora | sort hora asc""",
    ...
    "tup_total_por_hora": """fields @timestamp | stats count(*) as count by bin(1h) as hora | sort hora asc""",
    "tup_resumen": """fields @timestamp | stats count(*) as count, max(@timestamp) as ultima_transaccion""",
    "tup_total_10m": """fields @timestamp | stats count(*) as count by bin(10m) as hora | sort hora asc""",
    "tup_errores_10m": """fields @timestamp | filter ispresent(MessageOut) and LogLevel = 'Error' | stats count(*) as count by bin(10m) as hora | sort hora asc""",
